...
class _string {
	...
	function hstrtoupper($str){
		if (is_array($str)){
			foreach ($str as $key => $val){
				$str[$key] = _string::hstrtoupper($val);
			}
		}else{
			$i=0;
			$total = strlen($str);
	...
	function hstrtolower($string){
		if (is_array($string)){
			foreach ($string as $key => $val){
				$string[$key] = _string::hstrtolower($val);
			}
		}else{
	...
	function haddslashes($string, $force = 0) {
		if(!MAGIC_QUOTES_GPC || $force) {
			if(is_array($string)) {
				foreach($string as $key => $val) {
					$string[$key] = _string::haddslashes($val, $force);
				}
			}else {
		...
		if(is_array($string)){
			while(@list($key,$var) = @each($string)) {
				if ($key != 'argc' && $key != 'argv' && (strtoupper($key) != $key || ''.intval($key) == "$key")) {
					...
					if (is_array($var))  {
						$string[$key] = _string::hstripslashes($var);
					}
				}
			}
		}else{
	...
	function getfirstletter($string) {
		if(WIKI_CHARSET=='UTF-8'){
			$string=_string::hiconv($string,'gbk','utf-8');
		}
		$dict=array(
	...
	function convert_to_unicode($string){
		if(WIKI_CHARSET=='GBK'){
			$string=_string::hiconv($string,'utf-8','gbk');
		}
		$string=preg_replace("/([\\xc0-\\xff][\\x80-\\xbf]*)/e","' U8'.bin2hex( \"$1\" )",_string::hstrtolower( $string ));
		if(strlen($string)<4){
